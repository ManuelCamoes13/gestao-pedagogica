<template>
  <div class="tables-basic">
    <b-row>
      <b-col>
    <h2 class="page-title"><span class="fw-semi-bold">Curso: </span> {{curso.descricao}}</h2>
    </b-col>
    <b-col>
      <div class="d-flex">
              <b-button v-b-modal.novo variant="inverse" class="mr-3" size="sm"><i class="fa fa-plus"></i> Adicionar Cadeiras</b-button>
    ...
    <b-form-input placeholder="Procurar" size="sm" @input="search" />
          ...
          <div class="table-resposive">
            <table class="table">
              ...
              <tbody>
                <tr v-for="cadeira in CursoCadeira"
                 :key="cadeira.id">
 ...
 saveData() {
   this.result.push(this.selectedCadeira.id);
     ...
     getData() {
      ...
      http
        .get(`/curso/${this.$route.params.idCurso}`, config)
        .then((result) => {
          this.curso = result.data.data;
        })
        .catch((error) => {
          error;
          this.loading = false;
